...
export function PublicTournamentPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    if (!supabase || !tournament?.id) return undefined;
    const channel = supabase.channel(`public-tournament:${tournament.id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'matches', filter: `tournament_id=eq.${tournament.id}` }, () => { void load(); }).subscribe();
    return () => { void supabase?.removeChannel(channel); };
  }, [load, tournament?.id]);
