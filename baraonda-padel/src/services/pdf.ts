...
export function exportPlayersPdf(tournament: Tournament) {
  ...
  autoTable(doc, { startY: 32, styles: { fontSize: 9, cellPadding: 2 }, headStyles: { fillColor: [30, 41, 59] }, head: [['Nome Cognome', 'Livello', 'Genere', 'Stato', 'Disponibilità', 'Note']], body: players.map(player => [
    fullName(player), player.level, player.gender, PLAYER_STATUS_LABELS[player.status] ?? player.status,
