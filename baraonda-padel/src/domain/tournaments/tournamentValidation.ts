...
export function validateTournament(values: TournamentFormValues): TournamentFormErrors {
  ...
  if (!Number.isFinite(values.targetMatchesPerPlayer) || values.targetMatchesPerPlayer <= 0) errors.targetMatchesPerPlayer = 'Il numero di partite deve essere maggiore di zero.';
