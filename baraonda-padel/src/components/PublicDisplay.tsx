...
const isToPlay = (match: Match) => match.status !== 'completed' && match.status !== 'cancelled' && !isMatchCompleted(match);

export function selectPublicLiveMatches(matches: Match[]) {
  const toPlay = matches.filter(isToPlay);
  const featured = matches.find(isLive) ?? toPlay[0] ?? matches[0];
  return { featured, upcoming: toPlay.filter(match => match.id !== featured?.id).slice(0, 3) };
}
...
export const PublicDisplay = memo(function PublicDisplay({ tournament, standings, reloadTournament, storageKey, view = 'live', onViewChange }: Props) {
  ...
  const { featured, upcoming } = useMemo(() => selectPublicLiveMatches(tournament.matches), [tournament.matches]);
