...
export function TournamentForm({ mode, tournament, busy = false, mutationError, onSubmit, onCancel, onDirtyChange }: {
  ...
  return <>
    ...
    <section className="form tournament-form">
      ...
      <label>Partite per giocatore<input type="number" min="1" value={values.targetMatchesPerPlayer} onChange={event => patch('targetMatchesPerPlayer', Number(event.target.value))} />{fieldError('targetMatchesPerPlayer')}</label>
    ...
    <div className="form-actions"><button type="button" className="secondary" disabled={busy} onClick={cancel}>Annulla</button><button type="button" disabled={busy || (mode === 'edit' && !changes.hasChanges)} onClick={requestSave}>{busy ? 'Salvataggio in corso…' : 'Salva torneo'}</button></div>
