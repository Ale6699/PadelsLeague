...
import { AdvantageTeam, LiveMatchScore, LiveMatchState, LiveScoreValidationResult, Match, MatchStatus, Tournament, fullName } from '../models';
...
export function MatchDashboard({ tournament, match, index, onClose, onPersist, onFinish, onReset }: Props) {
  ...
  const dashboard = useMatchDashboard({ match, durationMinutes: tournament.settings.playMinutes || 12, maxGames, onPersist });
  ...
  useEffect(() => {
    const shortcut = (event: KeyboardEvent) => {
      ...
      else if (event.key === ' ') { event.preventDefault(); dashboard.live.timer.status === 'running' ? dashboard.pauseTimer() : dashboard.startTimer(); }
  ...
  return <div className="match-dashboard">
    <header className="dashboard-header">
      ...
      <div className="dashboard-status"><span>{statusLabel[dashboard.status]}</span><button className="secondary" onClick={() => void toggleFullscreen()}>{document.fullscreenElement ? <Minimize /> : <Expand />} Schermo intero</button></div>
    </header>
    <main className="dashboard-main">
      <div className={`timer-display timer-${dashboard.live.timer.status}`}>
        <small>{dashboard.live.timer.status === 'expired' ? 'TEMPO SCADUTO' : statusLabel[dashboard.status]}</small>
        <strong>{formatRemainingTime(dashboard.remainingMilliseconds)}</strong>
        <span>{dashboard.live.servingTeam === 'team_a' ? 'Servizio: Coppia A' : 'Servizio: Coppia B'}</span>
        {dashboard.live.timer.status === 'expired' && unfinishedGame && <em>{unfinishedGame}</em>}
      </div>
      <div className="scoreboard">
        <TeamScorePanel team="A" players={[names.get(match.players[0]) ?? '—', names.get(match.players[1]) ?? '—']} games={dashboard.live.score.teamAGames} points={dashboard.live.score.teamAPoints} hasAdvantage={dashboard.live.score.advantageTeam === 'team_a'} serving={dashboard.live.servingTeam === 'team_a'} disabled={completed} onPoint={() => award('team_a')} />
      ...
      <section className="score-history"><h2>Ultime azioni</h2>{dashboard.live.history.length ? dashboard.live.history.slice(-5).reverse().map(action => <p key={action.id}>{new Date(action.timestamp).toLocaleTimeString('it-IT')} · {action.type.replaceAll('_', ' ')}</p>) : <p>Nessuna azione</p>}</section>
    </main>
  </div>;
}
