...
export function Dashboard({ tournament, exportPdf, onOpenDashboard, onEdit, onDelete }: { tournament: Tournament; exportPdf: () => void; onOpenDashboard: (matchId: string) => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <>
    ...
    <section className="content-panel"><h2>Distribuzione partite</h2><div className="distribution">{[...tournament.players].sort((a, b) => fullName(a).localeCompare(fullName(b), 'it', { sensitivity: 'base' })).map(player =><div key={player.id}><span>{fullName(player)}</span><div className="bar"><i style={{ width: `${Math.min(100, (counts.get(player.id) ?? 0) / Math.max(1, tournament.settings.targetMatchesPerPlayer) * 100)}%` }} /></div><strong>{counts.get(player.id) ?? 0}</strong></div>)}</div></section>
